package com.pepej.papi.function.chain;
...
final class SimpleChain<T> implements Chain<T> {
    ...
    public <R> Chain<R> ifElse(Predicate<? super T> test, R passValue, R failValue) {
        Objects.requireNonNull(test, "test");
        return test.test(this.object) ? map($ -> passValue) : map($ -> failValue);
    }
    ...
    public <R> Chain<R> mapOrElse(Predicate<? super T> test, Function<? super T, ? extends R> passedMapper, R otherValue) {
        ...
        return mapOrElse(test, passedMapper, $ -> otherValue);
    }
