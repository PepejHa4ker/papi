...
public class VectorNl implements Vectorl, Comparable<VectorNl>, Serializable, Cloneable {
    ...
    public long lengthSquared(VectorNl v) {
        return lengthSquared(v.vec);
    }

    public long lengthSquared(long... v) {
        final int size = size();
        if (size != v.length) {
            throw new IllegalArgumentException("Vector sizes must be the same");
        }
    ...
    public long length(VectorNl v) {
        return length(v.vec);
    }

    public long length(long... v) {
        return (long) Math.sqrt(lengthSquared(v));
    }
    ...
    public String toString() {
        return "VectorNl" + Arrays.toString(vec).replace('[', '(').replace(']', ')');
    }
