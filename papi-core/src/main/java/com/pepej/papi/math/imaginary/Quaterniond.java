...
public class Quaterniond implements Imaginaryd, Comparable<Quaterniond>, Serializable, Cloneable {
    ...
    public double dot(float x, float y, float z, float w) {
        return dot(x, y, z, (double) w);
    }

    /**
     * Returns the dot product of this quaternion with the doubles components of another one.
     *
     * @param x The x (imaginary) component of the quaternion to calculate the dot product with
     * @param y The y (imaginary) component of the quaternion to calculate the dot product with
     * @param z The z (imaginary) component of the quaternion to calculate the dot product with
    ...
    public Vector3d getDirection() {
        return Matrix3d.createRotation(this).transform(Vector3d.FORWARD);
    }
...
@Override
    ...
    public static Quaterniond fromAxesAnglesDeg(float pitch, float yaw, float roll) {
        return fromAxesAnglesDeg(pitch, yaw, (double) roll);
    }
    ...
    public static Quaterniond fromAxesAnglesRad(float pitch, float yaw, float roll) {
        return fromAxesAnglesRad(pitch, yaw, (double) roll);
    }
    ...
    public static Quaterniond fromAxesAnglesDeg(double pitch, double yaw, double roll) {
        return Quaterniond.fromAngleDegAxis(yaw, Vector3d.UP).
                mul(Quaterniond.fromAngleDegAxis(pitch, Vector3d.RIGHT)).
                mul(Quaterniond.fromAngleDegAxis(roll, Vector3d.FORWARD));
    }
    ...
    public static Quaterniond fromAxesAnglesRad(double pitch, double yaw, double roll) {
        return Quaterniond.fromAngleRadAxis(yaw, Vector3d.UP).
                mul(Quaterniond.fromAngleRadAxis(pitch, Vector3d.RIGHT)).
                mul(Quaterniond.fromAngleRadAxis(roll, Vector3d.FORWARD));
    ...
    public static Quaterniond fromRotationTo(Vector3d from, Vector3d to) {
        return Quaterniond.fromAngleRadAxis(TrigonometricMath.acos(from.dot(to) / (from.length() * to.length())), from.cross(to));
    }
    ...
    public static Quaterniond fromAngleRadAxis(float angle, float x, float y, float z) {
        return fromAngleRadAxis(angle, x, y, (double) z);
    }
    ...
    public static Quaterniond fromAngleRadAxis(double angle, double x, double y, double z) {
        ...
        final double q = TrigonometricMath.sin(halfAngle) / Math.sqrt(x * x + y * y + z * z);
        return new Quaterniond(x * q, y * q, z * q, TrigonometricMath.cos(halfAngle));
    }
        ...
        if (trace < 0) {
            if (matrix.get(1, 1) > matrix.get(0, 0)) {
                if (matrix.get(2, 2) > matrix.get(1, 1)) {
                    final double r = Math.sqrt(matrix.get(2, 2) - matrix.get(0, 0) - matrix.get(1, 1) + 1);
                    ...
                    final double r = Math.sqrt(matrix.get(1, 1) - matrix.get(2, 2) - matrix.get(0, 0) + 1);
                    final double s = 0.5f / r;
                    return new Quaterniond(
                ...
                final double r = Math.sqrt(matrix.get(0, 0) - matrix.get(1, 1) - matrix.get(2, 2) + 1);
                final double s = 0.5f / r;
                return new Quaterniond(
                        0.5f * r,
                        (matrix.get(0, 1) + matrix.get(1, 0)) * s,
            ...
            final double r = Math.sqrt(trace + 1);
            final double s = 0.5f / r;
            return new Quaterniond(
                    (matrix.get(2, 1) - matrix.get(1, 2)) * s,
                    (matrix.get(0, 2) - matrix.get(2, 0)) * s,
