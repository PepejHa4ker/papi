...
public class VectorNf implements Vectorf, Comparable<VectorNf>, Serializable, Cloneable {
    ...
    public float lengthSquared(VectorNf v) {
        return lengthSquared(v.vec);
    }

    public float lengthSquared(float... v) {
        final int size = size();
        if (size != v.length) {
            throw new IllegalArgumentException("Vector sizes must be the same");
        }
    ...
    public float length(VectorNf v) {
        return length(v.vec);
    }
    ...
    public String toString() {
        return "VectorNf" + Arrays.toString(vec).replace('[', '(').replace(']', ')');
    }
