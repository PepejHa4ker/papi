...
public class VectorNd implements Vectord, Comparable<VectorNd>, Serializable, Cloneable {
    ...
    public VectorNd pow(double power) {
        ...
        final VectorNd d = new VectorNd(size);
        for (int comp = 0; comp < size; comp++) {
            d.vec[comp] = Math.pow(vec[comp], power);
        }
        return d;
    }
    ...
    public double lengthSquared(VectorNd v) {
        return lengthSquared(v.vec);
    }

    public double lengthSquared(double... v) {
        final int size = size();
        if (size != v.length) {
            throw new IllegalArgumentException("Vector sizes must be the same");
        }
    ...
    public double length(VectorNd v) {
        return length(v.vec);
    }
    ...
    public String toString() {
        return "VectorNd" + Arrays.toString(vec).replace('[', '(').replace(']', ')');
    }
