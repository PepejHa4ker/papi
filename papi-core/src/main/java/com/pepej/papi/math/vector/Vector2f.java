...
public class Vector2f implements Vectorf, Comparable<Vector2f>, Serializable, Cloneable {
    ...
    public float lengthSquared(double x, double y) {
        return lengthSquared((float) x, (float) y);
    }

    public float lengthSquared(float x, float y) {
    ...
    public float length(double x, double y) {
        return length((float) x, (float) y);
    }
    ...
    public static Vector2f createDirectionRad(float angle) {
        return new Vector2f(TrigonometricMath.cos(angle), TrigonometricMath.sin(angle));
    }
}
