...
public class Vector2i implements Vectori, Comparable<Vector2i>, Serializable, Cloneable {
    ...
    public int lengthSquared(Vector2i v) {
        return lengthSquared(v.x, v.y);
    }

    public int lengthSquared(double x, double y) {
        return lengthSquared((int) x, (int) y);
    }

    public int lengthSquared(int x, int y) {
        final int dx = this.x - x;
        final int dy = this.y - y;
        return dx * dx + dy * dy;
    }

    public int length(Vector2i v) {
        return length(v.x, v.y);
    }

    public int length(double x, double y) {
        return length((int) x, (int) y);
    }
