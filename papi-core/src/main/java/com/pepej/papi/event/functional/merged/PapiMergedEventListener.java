...
class PapiMergedEventListener<T> implements MergedSubscription<T>, EventExecutor, Listener {
    ...
    PapiMergedEventListener(MergedSubscriptionBuilderImpl<T> builder, List<BiConsumer<MergedSubscription<T>, ? super T>> handlers) {
    ...
    void register(Plugin plugin) {
        ...
        for (Map.Entry<Class<? extends Event>, MergedHandlerMapping<T, ? extends Event>> ent : this.mappings.entrySet()) {
            ...
            Papi.plugins().registerEvent(registrationType, this, ent.getValue().getPriority(), this, plugin, false);
        }
    }
